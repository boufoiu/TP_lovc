...
void ecrire_enr_dans_bloc(lovc*f , Entete* head , enrengistrement* enr , maillon* buf){
    ...
    enr_to_string(*enr,&copy);
        if(strlen(copy)<(TAILLE_BLOC-head->position_dns_dernier_bloc)){
            if(head->position_dns_dernier_bloc == 0){
                strcpy(buf->tab,copy);
            }           
            else{
                strcat(buf->tab,copy);
            ...
            buf->suivant = -1;
            if(head->position_dns_dernier_bloc==(TAILLE_BLOC-1)){   
                buf->suivant = head->adr_dernier_bloc + 1;             
                EcrireDir(f,head->adr_dernier_bloc,buf);
                if(head->nbbloc != head->adr_dernier_bloc){
                    LireDir(f,head->nbbloc,buf);
                    buf->suivant = head->adr_dernier_bloc;
                ...
                head->position_dns_dernier_bloc = 0;                
            }       
...
void main(){ 
    ...
    srand(time(NULL));
    lovc* fich = Ouvrir(fich,"moula.bin",'n');
    maillon buf;
    Entete head;
    copier_entete(fich,&head);
    ...
    //enr_to_string(enr,&copy);
    //printf("%s \n",copy);
    ecrire_enr_dans_bloc(fich,&head,&enr,&buf);
    printf("enregistrement ecrit \n");
    EcrireDir(fich,1,&buf);
    //afficher_bloc(fich,1);
    afficher_bloc(fich,1);
    Aff_entete(fich,1,head.tete);
    Aff_entete(fich,2,head.nb_ins);
    Aff_entete(fich,3,head.nb_car_sup);
    Aff_entete(fich,4,head.adr_dernier_bloc);
    Aff_entete(fich,5,head.position_dns_dernier_bloc);
    Aff_entete(fich,6,head.nbbloc); 
    
    affichier_entete(fich);
    fermer(fich);
...
/*/ afiichier un fichier lovc
void affichier_lovc(lovc* f){
}*/

/*/creation du fichier r « Livrets_National.bin »
lovc* creation_fichier(){
    lovc* f = malloc(sizeof(lovc));
    Ouvrir(f,"Livrets_National.bin",'N');
    enrengistrement* enr = malloc(sizeof(enrengistrement));
    maillon* buf = malloc(sizeof(maillon));
    Entete* head = malloc(sizeof(Entete));
    head->tete = 1;
    head->nb_ins = 0;
    head->nb_car_sup = 0;
    head->adr_dernier_bloc = 1;
    head->position_dns_dernier_bloc = 0;
    head->nbbloc = 1;    
    char* superficie;
    char** tab_wilaya;
    ...
    char* copy;
    char* observation;
    long long int cle = MIN_CLE ; 
    int nb_livrets = alea_nb(MIN_NB_LIVRETS_ALEATOIRES,MAX_NB_LIVRETS_ALEATOIRES);
    int position = 0 ;    //position dans le tableau de buf 
    int i ; // position dans copy 
    for (int i = 0; i < nb_livrets; i++)
    { 
        enr->effacer = 0 ;
        cle = alea_nb(cle,MAX_CLE);
        enr->cle = cle;
        wilaya = alea_wilaya(tab_wilaya);
        strcpy(enr->wilaya,wilaya);
        enr->type = alea_type();
        alea_superficie(superficie);
        strcpy(enr->superficie,superficie);
        observation = alea_observation();
        strcpy(enr->observation,observation);
        enr_to_string(enr,&copy);
        if(strlen(copy)<=(TAILLE_BLOC-position)){
            ...
            for(i = 0 ; i < TAILLE_BLOC-position-1 ; i++){
                buf->tab[position] = copy[i];
                position++;
            }
            buf->tab[position]='\0';
